...
struct NodeConfig {
    ...
    std::filesystem::path failure_capture_dir =
        "/tmp/wadjet_failures";  ///< Directory to save failure PCAPs
};
...
public:
    ...
    virtual auto wait_at_barrier(const std::string& barrier_id,
                                 std::chrono::milliseconds timeout) -> Result<BarrierResult> = 0;
    ...
    virtual auto evaluate_matcher(const std::string& matcher_type,
                                  const std::string& matcher_config) -> Result<std::string> = 0;
