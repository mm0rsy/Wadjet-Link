...
public:
     ...
     */
    auto add_capture(const std::string& node_id, const std::filesystem::path& pcap_path)
        -> wadjet::Result<void>;
     ...
     * @return Success or error
     */
    auto add_packets(const std::string& node_id, const std::vector<Packet>& packets)
        -> wadjet::Result<void>;
     ...
     */
    auto merge(const std::filesystem::path& output_path)
