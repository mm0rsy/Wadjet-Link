...
struct DTCRecord {
    ...
    [[nodiscard]] std::string dtc_string() const { return dtc.to_string(); }

    /// @brief Check if DTC is currently active
    [[nodiscard]] bool is_active() const { return status.test_failed; }

    /// @brief Check if DTC is confirmed
    [[nodiscard]] bool is_confirmed() const { return status.confirmed_dtc; }

    /// @brief Check if DTC is pending
    [[nodiscard]] bool is_pending() const { return status.pending_dtc; }

    /// @brief Check if DTC failed since last clear
    [[nodiscard]] bool failed_since_clear() const { return status.test_failed_since_last_clear; }
...
using DTCEventCallback =
    std::function<void(DTCEvent event, LogicalAddress ecu_address,
...
public:
    ...
    void process_read_dtc_response(
        const uds::ReadDTCInformationResponse& resp, LogicalAddress ecu_address,
        std::chrono::steady_clock::time_point timestamp = std::chrono::steady_clock::now());

    /// @brief Process ClearDiagnosticInformation response
    void process_clear_dtc_response(
        const uds::ClearDiagnosticInformationRequest& req, LogicalAddress ecu_address, bool success,
        std::chrono::steady_clock::time_point timestamp = std::chrono::steady_clock::now());

    /// @brief Add or update a single DTC
    void add_dtc(
        const uds::DTC& dtc, const uds::DTCStatusMask& status, LogicalAddress ecu_address,
        std::chrono::steady_clock::time_point timestamp = std::chrono::steady_clock::now());

    /// @brief Add snapshot data to a DTC
    void add_snapshot(
        const uds::DTC& dtc, LogicalAddress ecu_address, std::uint8_t record_number,
        std::span<const std::uint8_t> data,
        std::chrono::steady_clock::time_point timestamp = std::chrono::steady_clock::now());

    /// @brief Add extended data to a DTC
    void add_extended_data(
        const uds::DTC& dtc, LogicalAddress ecu_address, std::span<const std::uint8_t> data,
        std::chrono::steady_clock::time_point timestamp = std::chrono::steady_clock::now());
    ...
    [[nodiscard]] const DTCRecord* get_dtc(const uds::DTC& dtc, LogicalAddress ecu_address) const;
