...
struct FlashSequence {
    ...
    [[nodiscard]] double progress() const {
        if (region.size == 0)
            return 0.0;
        return static_cast<double>(total_bytes_transferred) / static_cast<double>(region.size);
    }

    /// @brief Get progress as percentage (0 - 100)
    [[nodiscard]] int progress_percent() const { return static_cast<int>(progress() * 100.0); }
    ...
    [[nodiscard]] bool is_active() const {
        return state == FlashSequenceState::Downloading ||
               state == FlashSequenceState::Transferring || state == FlashSequenceState::Erasing ||
               state == FlashSequenceState::Verifying;
    }

    /// @brief Check if transfer completed successfully
    [[nodiscard]] bool is_successful() const { return state == FlashSequenceState::Complete; }

    /// @brief Check if transfer failed
    [[nodiscard]] bool is_failed() const {
        return state == FlashSequenceState::Failed || state == FlashSequenceState::Aborted ||
    ...
    [[nodiscard]] std::size_t successful_blocks() const {
        ...
        for (const auto& block : blocks) {
            if (block.acknowledged && !block.error_code)
                count++;
        }
        return count;
    }
...
using FlashEventCallback = std::function<void(FlashEvent event, const FlashSequence& sequence)>;
...
public:
    ...
    void process_request_download_response(
        const uds::RequestDownloadResponse& resp, LogicalAddress source, LogicalAddress target,
        std::chrono::steady_clock::time_point timestamp = std::chrono::steady_clock::now());

    /// @brief Process RequestUpload request
    void process_request_upload(
        const uds::RequestUploadRequest& req, LogicalAddress source, LogicalAddress target,
        std::chrono::steady_clock::time_point timestamp = std::chrono::steady_clock::now());

    /// @brief Process RequestUpload response
    void process_request_upload_response(
        const uds::RequestUploadResponse& resp, LogicalAddress source, LogicalAddress target,
        std::chrono::steady_clock::time_point timestamp = std::chrono::steady_clock::now());

    /// @brief Process TransferData request
    void process_transfer_data(
        const uds::TransferDataRequest& req, LogicalAddress source, LogicalAddress target,
        std::chrono::steady_clock::time_point timestamp = std::chrono::steady_clock::now());

    /// @brief Process TransferData response
    void process_transfer_data_response(
        const uds::TransferDataResponse& resp, LogicalAddress source, LogicalAddress target,
        std::chrono::steady_clock::time_point timestamp = std::chrono::steady_clock::now());

    /// @brief Process RequestTransferExit request
    void process_transfer_exit(
        const uds::RequestTransferExitRequest& req, LogicalAddress source, LogicalAddress target,
        std::chrono::steady_clock::time_point timestamp = std::chrono::steady_clock::now());

    /// @brief Process RequestTransferExit response
    void process_transfer_exit_response(
        const uds::RequestTransferExitResponse& resp, LogicalAddress source, LogicalAddress target,
        std::chrono::steady_clock::time_point timestamp = std::chrono::steady_clock::now());

    /// @brief Process negative response related to flash
    void process_negative_response(
        const uds::NegativeResponseMessage& nrc, LogicalAddress source, LogicalAddress target,
        std::chrono::steady_clock::time_point timestamp = std::chrono::steady_clock::now());

    /// @brief Process RoutineControl for erase/verification
    void process_routine_control(
        const uds::RoutineControlRequest& req, LogicalAddress source, LogicalAddress target,
        std::chrono::steady_clock::time_point timestamp = std::chrono::steady_clock::now());

    /// @brief Process RoutineControl response
    void process_routine_control_response(
        const uds::RoutineControlResponse& resp, LogicalAddress source, LogicalAddress target,
        std::chrono::steady_clock::time_point timestamp = std::chrono::steady_clock::now());
