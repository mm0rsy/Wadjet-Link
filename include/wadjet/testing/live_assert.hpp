...
public:
    ...
    AssertionRule(Type type, std::string description, 
                  PacketPredicate predicate,
                  std::optional<PacketPredicate> condition = std::nullopt)
        : type_(type), description_(std::move(description)),
          predicate_(std::move(predicate)), condition_(std::move(condition)) {}
...
public:
    ConditionalAssertionBuilder(std::vector<AssertionRule>& rules,
        ...
        : rules_(rules), condition_(std::move(condition)),
    ...
    ConditionalAssertionBuilder& assert_that(const Matcher& matcher) {
        ...
        rules_.emplace_back(
            AssertionRule::Type::ASSERT_WHEN,
            desc.str(),
...
public:
    ...
    explicit LiveAssertSession(const std::string& interface = "lo")
    ...
    template <typename Matcher>
    LiveAssertSession& assert_all(const Matcher& matcher,
                                   const std::string& desc = "") {
        std::string description = desc.empty() ? "All packets must match" : desc;
        rules_.emplace_back(
            AssertionRule::Type::ASSERT_ALL,
            description,
            [matcher](const Packet& pkt) { return ::testing::Value(pkt, matcher); }
        );
        return *this;
    ...
    template <typename Matcher>
    LiveAssertSession& assert_never(const Matcher& matcher,
                                     const std::string& desc = "") {
        std::string description = desc.empty() ? "No packet should match" : desc;
        rules_.emplace_back(
            AssertionRule::Type::ASSERT_NEVER,
            description,
            [matcher](const Packet& pkt) { return ::testing::Value(pkt, matcher); }
        );
        return *this;
    ...
    template <typename Matcher>
    LiveAssertSession& expect_within(const Matcher& matcher,
                                      const std::string& desc = "") {
        std::string description = desc.empty() ? "At least one packet must match" : desc;
        rules_.emplace_back(
            AssertionRule::Type::EXPECT_WITHIN,
            description,
            [matcher](const Packet& pkt) { return ::testing::Value(pkt, matcher); }
        );
        return *this;
                ...
                if (stop_condition(*packet)) {
                    ...
                    for (auto& rule : rules_) {
                        if (rule.type() == AssertionRule::Type::EXPECT_WITHIN && 
                            ...
                            fail_result.description = "Expected packet not found: " + 
                                                     rule.description();
                            save_failure_trace();
                            return fail_result;
                        }
    ...
    [[nodiscard]] const std::vector<Packet>& captured_packets() const {
...
private:
    ...
    void save_failure_trace() {
        if (!save_on_failure_ || captured_packets_.empty()) return;
