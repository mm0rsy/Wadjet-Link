...
protected:
    ...
    std::optional<Packet> wait_for_match(const Matcher& matcher,
                                         std::chrono::milliseconds timeout) {
        return wait_for_packet([&matcher](const Packet& pkt) {
    ...
    template <typename Predicate>
    std::vector<Packet> collect_until(Predicate&& predicate,
                                      std::chrono::milliseconds timeout) {
        std::vector<Packet> packets;
        auto deadline = std::chrono::steady_clock::now() + timeout;
...
protected:
    ...
    void save_failure_pcap() {
        ...
        std::string filename = std::string(test_info->test_suite_name()) + "_" +
