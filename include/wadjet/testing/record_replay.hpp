...
public:
    ...
    explicit RecordedStream(std::vector<Packet> packets) : packets_(std::move(packets)) {}
    ...
    [[nodiscard]] RecordedStream filter_matches(const Matcher& matcher) const {
        return filter([&matcher](const Packet& pkt) { return ::testing::Value(pkt, matcher); });
    }
    ...
    [[nodiscard]] bool filter_any_matches(const Matcher& matcher) const {
        return std::any_of(packets_.begin(), packets_.end(), [&matcher](const Packet& pkt) {
    ...
    [[nodiscard]] bool filter_all_match(const Matcher& matcher) const {
        return std::all_of(packets_.begin(), packets_.end(), [&matcher](const Packet& pkt) {
            return ::testing::Value(pkt, matcher);
        });
    }
    ...
    [[nodiscard]] std::optional<std::reference_wrapper<const Packet>> find_first(
        PacketPredicate predicate) const {
    ...
    template <typename Matcher>
    [[nodiscard]] std::optional<std::reference_wrapper<const Packet>> filter_find_first(
        const Matcher& matcher) const {
        auto it = std::find_if(packets_.begin(), packets_.end(), [&matcher](const Packet& pkt) {
            return ::testing::Value(pkt, matcher);
        });
        if (it == packets_.end())
            return std::nullopt;
    ...
    [[nodiscard]] std::optional<std::size_t> find_index(PacketPredicate predicate) const {
        auto it = std::find_if(packets_.begin(), packets_.end(), std::move(predicate));
        if (it == packets_.end())
            return std::nullopt;
    ...
    [[nodiscard]] RecordedStream take(std::size_t n) const {
        ...
        return RecordedStream(std::vector<Packet>(
            packets_.begin(), packets_.begin() + static_cast<std::ptrdiff_t>(count)));
    }
    ...
    [[nodiscard]] RecordedStream skip(std::size_t n) const {
        ...
        return RecordedStream(
            std::vector<Packet>(packets_.begin() + static_cast<std::ptrdiff_t>(n), packets_.end()));
    }
    ...
    [[nodiscard]] RecordedStream slice(std::size_t start, std::size_t end) const {
        ...
        if (start >= end)
            return RecordedStream();
        return RecordedStream(
            std::vector<Packet>(packets_.begin() + static_cast<std::ptrdiff_t>(start),
    ...
    [[nodiscard]] bool all_consecutive_satisfy(BinaryPredicate pred) const {
        ...
        for (std::size_t i = 0; i + 1 < packets_.size(); ++i) {
            if (!pred(packets_[i], packets_[i + 1]))
                return false;
        }
        return true;
    }
...
public:
    ...
    explicit RecordSession(const std::string& interface = "lo") : interface_(interface) {}
    ...
    bool record_until(Predicate&& predicate, std::chrono::duration<Rep, Period> timeout) {
        auto session = io::CaptureSession::create(interface_);
        if (!session.is_ok())
            return false;
...
#define WADJET_ASSERT_ALL_MATCH(stream, matcher) \
    ASSERT_TRUE((stream).filter_all_match(matcher)) << "Expected all packets to match: " << #matcher

/// Expect that all packets in the stream match the matcher
#define WADJET_EXPECT_ALL_MATCH(stream, matcher) \
    EXPECT_TRUE((stream).filter_all_match(matcher)) << "Expected all packets to match: " << #matcher
