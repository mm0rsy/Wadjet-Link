...
auto ConfigLoader::load_nodes(const std::filesystem::path& config_path)
    ...
    if (!std::filesystem::exists(config_path)) {
        return Result<std::vector<NodeInfo>>(
            Error::make("CONFIG_NOT_FOUND", "Configuration file not found: " + config_path.string())
        );
    }
...
auto ConfigLoader::load_nodes_yaml(const std::filesystem::path& config_path)
    ...
    try {
        ...
        return Result<std::vector<NodeInfo>>(
            Error::make("CONFIG_LOAD_ERROR", "Failed to load configuration: " + std::string(e.what()))
        );
    }
}
...
auto ConfigLoader::parse_yaml_node(void* node_entry) -> Result<NodeInfo> {
    try {
        ...
        if (!(*node)["hostname"]) {
            return Result<NodeInfo>(Error::make("MISSING_FIELD", "Node must have 'hostname' field"));
        }
        info.hostname = (*node)["hostname"].as<std::string>();
        
        if (!(*node)["grpc_port"]) {
            return Result<NodeInfo>(Error::make("MISSING_FIELD", "Node must have 'grpc_port' field"));
        }
        ...
        return Result<NodeInfo>(
            Error::make("YAML_PARSE_ERROR", "Failed to parse node entry: " + std::string(e.what()))
        );
    } catch (const std::exception& e) {
        return Result<NodeInfo>(
...
auto ConfigLoader::load_nodes_json(const std::filesystem::path& config_path)
    ...
    try {
        ...
        if (!file.is_open()) {
            return Result<std::vector<NodeInfo>>(
                Error::make("FILE_OPEN_ERROR", "Failed to open configuration file: " + config_path.string())
            );
        }
...
auto ConfigLoader::parse_json_node(void* node_entry) -> Result<NodeInfo> {
    try {
        ...
        if (node->contains("capture_interfaces")) {
            info.capture_interfaces = node->at("capture_interfaces").get<std::vector<std::string>>();
        }
        ...
        return Result<NodeInfo>(
            Error::make("JSON_PARSE_ERROR", "Failed to parse node entry: " + std::string(e.what()))
        );
    } catch (const std::exception& e) {
        return Result<NodeInfo>(
            Error::make("NODE_PARSE_ERROR", "Failed to parse node: " + std::string(e.what()))
...
auto ConfigLoader::load_coordinator_config(const std::filesystem::path& config_path)
    ...
    try {
        ...
        return Result<CoordinatorConfig>(
            Error::make("CONFIG_LOAD_ERROR", "Failed to load coordinator config: " + std::string(e.what()))
        );
    }
}
