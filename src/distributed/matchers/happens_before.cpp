...
public:
    ...
    auto evaluate(const std::unordered_map<std::string, DistributedCaptureContext>& contexts)
        ...
        if (a_it == contexts.end()) {
            return DistributedMatchResult::failure("Node for event A '" + event_a_node_ +
                                                   "' has no captured packets");
        }

        if (b_it == contexts.end()) {
            return DistributedMatchResult::failure("Node for event B '" + event_b_node_ +
                                                   "' has no captured packets");
        }
        ...
        if (a_packets.empty()) {
            return DistributedMatchResult::failure("No packets captured on event A node '" +
                                                   event_a_node_ + "'");
        }

        if (b_packets.empty()) {
            return DistributedMatchResult::failure("No packets captured on event B node '" +
                                                   event_b_node_ + "'");
        }
        ...
        return DistributedMatchResult::failure(
            "Causality violation: event on '" + event_a_node_ + "' (at " +
            std::to_string(latest_a_timestamp) + "ns) does not happen before event on '" +
...
auto HappensBefore(std::string event_a_node, ::testing::Matcher<const PacketView&> event_a_matcher,
    ...
    public:
        GTestAwareHappensBefore(std::string a_node, ::testing::Matcher<const PacketView&> a_matcher,
                                std::string b_node, ::testing::Matcher<const PacketView&> b_matcher)
    ...
    return std::make_unique<GTestAwareHappensBefore>(
        std::move(event_a_node), std::move(event_a_matcher), std::move(event_b_node),
        std::move(event_b_matcher));
}
