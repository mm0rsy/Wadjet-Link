...
MessageCorrelator::MessageCorrelator(CorrelationMethod method) : impl_(std::make_unique<Impl>()) {
    impl_->method = method;
}

MessageCorrelator::MessageCorrelator(CorrelationFunc func) : impl_(std::make_unique<Impl>()) {
...
void MessageCorrelator::add_packets(const std::string& node_id, std::span<const Packet> packets) {
