...
// Constructor
PcapMerger::PcapMerger(const PcapMergerOptions& options) : impl_(std::make_unique<Impl>(options)) {}

// Destructor
PcapMerger::~PcapMerger() = default;

// Add a PCAP file from a node
auto PcapMerger::add_capture(const std::string& node_id,
                             const std::filesystem::path& pcap_path) -> wadjet::Result<void> {
    if (node_id.empty()) {
        return wadjet::Result<void>::err(wadjet::Error(-1, "Node ID cannot be empty"));
    }
...
auto PcapMerger::add_packets(const std::string& node_id,
...
auto PcapMerger::merge(const std::filesystem::path& output_path)
    ...
    for (auto& [node_id, node_cap] : impl_->node_captures) {
        for (size_t i = 0; i < node_cap.packets.size(); ++i) {
            all_packets.push_back({&node_cap.packets[i], node_id, i});
        }
    }
