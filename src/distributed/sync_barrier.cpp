...
// T011: Constructor
SyncBarrier::SyncBarrier(const std::string& barrier_id) : impl_(std::make_unique<Impl>()) {
...
auto SyncBarrier::arrive_and_wait(const std::string& node_id,
                                  std::chrono::milliseconds timeout) -> Result<BarrierResult> {
    std::unique_lock<std::mutex> lock(impl_->mutex);
    ...
    bool proceed = impl_->cv.wait_for(lock, timeout, [this]() { return impl_->node_proceed; });
