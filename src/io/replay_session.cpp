...
auto ReplaySession::create(const std::string& interface,
...
auto ReplaySession::init_socket() -> Result<void> {
    ...
    if (ioctl(impl_->socket_fd, SIOCGIFINDEX, &ifr) < 0) {
        ...
        impl_->socket_fd = -1;
        return Result<void>::err(
            Error{errno, "Failed to get interface index for: " + interface_});
    }
    ...
    if (bind(impl_->socket_fd, reinterpret_cast<sockaddr*>(&bind_addr),
             sizeof(bind_addr)) < 0) {
        close(impl_->socket_fd);
        impl_->socket_fd = -1;
        return Result<void>::err(
            Error{errno, "Failed to bind to interface: " + interface_});
    }
    ...
    if (options_.promiscuous) {
        ...
        setsockopt(impl_->socket_fd, SOL_PACKET, PACKET_ADD_MEMBERSHIP,
                   &mreq, sizeof(mreq));
...
#else
    return Result<void>::err(
        Error{-1, "AF_PACKET replay only supported on Linux"});
#endif
}
...
auto ReplaySession::send_packet(const PacketView& view) -> Result<void> {
    ...
    auto sent = sendto(impl_->socket_fd,
                       data.data(),
                       data.size(),
                       0,
                       reinterpret_cast<sockaddr*>(&impl_->socket_addr),
                       sizeof(impl_->socket_addr));
        ...
        case ReplayTiming::Scaled: {
            ...
            if (delta > 0 && options_.speed_factor > 0) {
                auto scaled = static_cast<std::int64_t>(
                    static_cast<double>(delta) / options_.speed_factor);
