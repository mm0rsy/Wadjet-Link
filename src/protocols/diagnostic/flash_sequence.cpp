...
std::string_view flash_sequence_state_string(FlashSequenceState state) {
    switch (state) {
        case FlashSequenceState::Idle: return "Idle";
        case FlashSequenceState::EraseStarted: return "EraseStarted";
        case FlashSequenceState::Erasing: return "Erasing";
        case FlashSequenceState::EraseComplete: return "EraseComplete";
        case FlashSequenceState::EraseFailed: return "EraseFailed";
        case FlashSequenceState::Downloading: return "Downloading";
        case FlashSequenceState::Transferring: return "Transferring";
        case FlashSequenceState::Verifying: return "Verifying";
        case FlashSequenceState::Complete: return "Complete";
        case FlashSequenceState::Failed: return "Failed";
        case FlashSequenceState::Aborted: return "Aborted";
    }
    return "Unknown";
}
...
std::string_view flash_event_string(FlashEvent event) {
    switch (event) {
        case FlashEvent::EraseStarted: return "EraseStarted";
        case FlashEvent::EraseProgress: return "EraseProgress";
        case FlashEvent::EraseCompleted: return "EraseCompleted";
        case FlashEvent::EraseFailed: return "EraseFailed";
        case FlashEvent::DownloadStarted: return "DownloadStarted";
        case FlashEvent::UploadStarted: return "UploadStarted";
        case FlashEvent::BlockTransferred: return "BlockTransferred";
        case FlashEvent::BlockAcknowledged: return "BlockAcknowledged";
        case FlashEvent::BlockFailed: return "BlockFailed";
        case FlashEvent::TransferExitOk: return "TransferExitOk";
        case FlashEvent::TransferExitFail: return "TransferExitFail";
        case FlashEvent::VerificationStart: return "VerificationStart";
        case FlashEvent::VerificationOk: return "VerificationOk";
        case FlashEvent::VerificationFail: return "VerificationFail";
        case FlashEvent::SequenceComplete: return "SequenceComplete";
        case FlashEvent::SequenceFailed: return "SequenceFailed";
        case FlashEvent::SequenceAborted: return "SequenceAborted";
    }
    return "Unknown";
}
...
FlashSequenceTracker::FlashSequenceTracker(Options opts)
...
void FlashSequenceTracker::process_request_download(
    const uds::RequestDownloadRequest& req,
...
void FlashSequenceTracker::process_request_download_response(
    ...
    [[maybe_unused]] LogicalAddress target,
    std::chrono::steady_clock::time_point timestamp) {
    std::lock_guard<std::mutex> lock(mutex_);
    ...
    // Update duration
    seq.total_duration = std::chrono::duration_cast<std::chrono::milliseconds>(
        timestamp - seq.start_time);
}

void FlashSequenceTracker::process_request_upload(
    const uds::RequestUploadRequest& req,
    LogicalAddress source,
    LogicalAddress target,
    std::chrono::steady_clock::time_point timestamp) {
    std::lock_guard<std::mutex> lock(mutex_);
...
void FlashSequenceTracker::process_transfer_data(
    const uds::TransferDataRequest& req,
    ...
    seq.expected_sequence_counter = static_cast<std::uint8_t>(
        (req.block_sequence_counter % 255) + 1);
...
void FlashSequenceTracker::process_negative_response(
    ...
    bool flash_related = (sid == uds::ServiceID::RequestDownload ||
                          sid == uds::ServiceID::RequestUpload ||
                          sid == uds::ServiceID::TransferData ||
                          sid == uds::ServiceID::RequestTransferExit);
    
    if (!flash_related) return;
...
void FlashSequenceTracker::process_routine_control(
    const uds::RoutineControlRequest& req,
    LogicalAddress source,
    LogicalAddress target,
    std::chrono::steady_clock::time_point timestamp) {
    std::lock_guard<std::mutex> lock(mutex_);
    ...
    if (!options_.track_erase_routines) return;
...
std::vector<FlashSequence> FlashSequenceTracker::get_history(LogicalAddress ecu,
    ...
    return std::vector<FlashSequence>(hist.end() - static_cast<std::ptrdiff_t>(count),
                                       hist.end());
}
