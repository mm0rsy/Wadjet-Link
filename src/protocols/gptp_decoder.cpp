...
GptpDecoder::Result 
GptpDecoder::decode_impl(const DecodeContext& ctx) const {
    if (ctx.data.size() < COMMON_HEADER_SIZE) {
        return make_error(DecodeErrorCode::BufferTooSmall,
                         "Insufficient data for gPTP header",
                         ctx.original_offset);
    }
    ...
    if (options_.strict_version && header->transport_specific != TransportSpecific::IEEE_802_1AS) {
        return make_error(DecodeErrorCode::InvalidVersion,
                         "Not an IEEE 802.1AS gPTP message",
                         ctx.original_offset);
    }
...
std::optional<GptpHeader>
GptpDecoder::parse_header(const std::byte* data, std::size_t len) const {
...
GptpDecoder::parse_body(MessageType type, const std::byte* data, std::size_t len) const {
    switch (type) {
    ...
    case MessageType::Announce: {
        ...
        announce.grandmaster_clock_quality.clock_class = 
            static_cast<std::uint8_t>(data[14]);
        announce.grandmaster_clock_quality.clock_accuracy = 
            static_cast<std::uint8_t>(data[15]);
