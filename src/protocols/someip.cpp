...
std::string SomeIpHeader::to_string() const {
    ...
    oss << "SOME/IP { service=0x" << std::hex << std::setfill('0')
        << std::setw(4) << service_id
        << ", method=0x" << std::setw(4) << method_id
        << ", client=0x" << std::setw(4) << client_id
        << ", session=0x" << std::setw(4) << session_id
        << std::dec
        << ", type=" << message_type_string(message_type)
        << ", ret=" << return_code_string(return_code)
        << ", len=" << length;
...
SomeIpDecoder::Result SomeIpDecoder::decode_impl(const DecodeContext& ctx) const {
    ...
    if (options_.validate_protocol_version &&
    ...
    header.message_type = static_cast<MessageType>(
        static_cast<std::uint8_t>(ctx.data[14]));

    // Return code (byte 15)
    header.return_code = static_cast<ReturnCode>(
        static_cast<std::uint8_t>(ctx.data[15]));

    // Validate length field
    if (header.length < 8) {
        return make_error(DecodeErrorCode::InvalidLength,
                          "SOME/IP length too small: " +
