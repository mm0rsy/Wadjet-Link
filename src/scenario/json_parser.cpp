...
Step parse_expect_step(const json& j) {
    ...
    if (j.contains("someip_sd") || j.contains("sd")) {
        step.someip_sd = parse_someip_sd_expect(
            j.contains("someip_sd") ? j["someip_sd"] : j["sd"]);
    }
    if (j.contains("doip")) {
    ...
    if (j.contains("count")) {
        if (j["count"].is_string()) {
            ...
            step.count = CountExpression{CompareOp::GreaterEqual, 
                                          j["count"].get<std::size_t>()};
        }
    }
...
Scenario parse_scenario(const json& root) {
    ...
    scenario.timeout = root.contains("timeout") ? 
        parse_duration(root["timeout"]) : Duration{30000};
