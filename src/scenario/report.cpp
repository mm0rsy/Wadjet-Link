...
void JUnitXmlReportGenerator::generate(std::ostream& out, const BatchResult& results,
                                       const ReportOptions& opts) {
    std::string hostname = opts.hostname.empty() ? get_hostname() : opts.hostname;
    std::string timestamp = get_timestamp();

    out << "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n";
    out << "<testsuites name=\"" << escape_xml(opts.suite_name) << "\"" << " tests=\""
        << results.total() << "\"" << " failures=\"" << results.failed << "\"" << " errors=\"0\""
        << " skipped=\"" << results.skipped << "\"" << " time=\""
        << duration_seconds(results.total_elapsed) << "\">\n";

    for (const auto& scenario_result : results.results) {
        std::size_t test_count =
            scenario_result.expect_results.empty() ? 1 : scenario_result.expect_results.size();
        std::size_t failures = scenario_result.failed_count();

        out << "  <testsuite name=\"" << escape_xml(scenario_result.scenario_name) << "\""
            << " tests=\"" << test_count << "\"" << " failures=\"" << failures << "\""
            << " errors=\"0\"" << " skipped=\"0\"";
        ...
        out << " hostname=\"" << escape_xml(hostname) << "\"" << " time=\""
            << duration_seconds(scenario_result.total_elapsed) << "\">\n";
        ...
        if (scenario_result.expect_results.empty()) {
            ...
            out << "    <testcase name=\"" << escape_xml(scenario_result.scenario_name) << "\""
                << " classname=\"" << escape_xml(opts.suite_name) << "\"" << " time=\""
                << duration_seconds(scenario_result.total_elapsed) << "\"";

            if (!scenario_result.passed) {
                out << ">\n";
                out << "      <failure message=\"" << escape_xml(scenario_result.error_message)
                    << "\"/>\n";
                out << "    </testcase>\n";
            } else {
                out << "/>\n";
            ...
            for (const auto& expect : scenario_result.expect_results) {
                ...
                if (!expect.passed) {
                    out << ">\n";
                    out << "      <failure message=\"" << escape_xml(expect.failure_reason) << "\">"
                        << "Matched " << expect.packets_matched << " packets" << "</failure>\n";
                    out << "    </testcase>\n";
                } else {
                    out << "/>\n";
                }
        ...
        if (opts.include_pcap_paths && scenario_result.pcap_file) {
            out << "    <system-out>PCAP saved: " << escape_xml(*scenario_result.pcap_file)
                << "</system-out>\n";
        }
...
void JUnitXmlReportGenerator::generate(std::ostream& out, const ScenarioResult& result,
                                       const ReportOptions& opts) {
    BatchResult batch;
...
void JsonReportGenerator::generate(std::ostream& out, const BatchResult& results,
                                   const ReportOptions& opts) {
    ...
    for (const auto& scenario_result : results.results) {
        ...
        out << indent << indent << indent
            << "\"passed\": " << (scenario_result.passed ? "true" : "false") << "," << newline;
        out << indent << indent << indent
            << "\"duration_ms\": " << scenario_result.total_elapsed.count() << "," << newline;
        ...
        for (const auto& expect : scenario_result.expect_results) {
            ...
            out << indent << indent << indent << indent << indent << "\"description\": \""
                << escape_json(expect.description) << "\"," << newline;
            ...
            if (!expect.passed && !expect.failure_reason.empty()) {
                out << "," << newline;
                out << indent << indent << indent << indent << indent << "\"failure_reason\": \""
                    << escape_json(expect.failure_reason) << "\"";
            }
...
void JsonReportGenerator::generate(std::ostream& out, const ScenarioResult& result,
                                   const ReportOptions& opts) {
    BatchResult batch;
    batch.results.push_back(result);
    batch.total_elapsed = result.total_elapsed;
...
void TextReportGenerator::generate(std::ostream& out, const BatchResult& results,
                                   const ReportOptions& opts) {
    ...
    for (const auto& scenario_result : results.results) {
        ...
        for (const auto& expect : scenario_result.expect_results) {
            ...
            out << expect_status << " " << expect.description << " (" << expect.packets_matched
                << " packets, " << expect.elapsed.count() << "ms)\n";
    ...
    out << "---\n";
    out << "Total: " << results.total() << " | Passed: " << results.passed
        << " | Failed: " << results.failed << " | Skipped: " << results.skipped << "\n";
    out << "Duration: " << results.total_elapsed.count() << "ms\n";
}

void TextReportGenerator::generate(std::ostream& out, const ScenarioResult& result,
                                   const ReportOptions& opts) {
    BatchResult batch;
    batch.results.push_back(result);
    batch.total_elapsed = result.total_elapsed;
...
void TapReportGenerator::generate(std::ostream& out, const BatchResult& results,
                                  const ReportOptions& /*opts*/) {
    std::size_t total_tests = 0;
    for (const auto& scenario : results.results) {
        total_tests += scenario.expect_results.empty() ? 1 : scenario.expect_results.size();
    }
...
void TapReportGenerator::generate(std::ostream& out, const ScenarioResult& result,
                                  const ReportOptions& opts) {
    BatchResult batch;
    batch.results.push_back(result);
    batch.total_elapsed = result.total_elapsed;
...
void generate_report(const std::filesystem::path& path, const BatchResult& results,
                     ReportFormat format, const ReportOptions& opts) {
...
std::string generate_report_string(const BatchResult& results, ReportFormat format,
                                   const ReportOptions& opts) {
    std::ostringstream ss;
    auto generator = create_report_generator(format);
