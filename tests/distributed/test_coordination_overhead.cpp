...
TEST_F(CoordinationOverheadTest, OverheadPercentageValidation) {
    ...
    int64_t total_grpc_overhead = grpc_heartbeat_overhead + 
                                  barrier_sync_overhead + 
                                  config_exchange_overhead;
    
    // Calculate overhead percentage
    double overhead_percent = (static_cast<double>(total_grpc_overhead) / 
                              estimated_test_traffic) * 100.0;
...
TEST_F(CoordinationOverheadTest, HeartbeatOverheadMinimal) {
    ...
    double heartbeat_overhead = (static_cast<double>(total_heartbeat_bytes) / 
                                 test_traffic) * 100.0;
