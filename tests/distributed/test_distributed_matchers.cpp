...
protected:
     ...
     */
    DistributedCaptureContext CreateContext(const std::string& node_id, size_t packet_count = 5,
...
TEST_F(ExpectMessageFlowTest, MissingSourceNode) {
    ...
    EXPECT_THAT(result.error_message, ::testing::HasSubstr("node-a"));
}
...
TEST_F(ExpectMessageFlowTest, MissingDestinationNode) {
    ...
    EXPECT_FALSE(result.matched);
    EXPECT_FALSE(result.error_message.empty());
    EXPECT_THAT(result.error_message, ::testing::HasSubstr("node-b"));
}
...
TEST_F(DistributedMatcherCompositionTest, WithinLatencyComposition) {
    ...
    EXPECT_THAT(with_latency->describe(), ::testing::HasSubstr("WithinLatency"));
}
...
TEST_F(M3MatcherCompositionTest, ExpectMessageFlowWithM3Matcher) {
    ...
    EXPECT_THAT(matcher_with_content->describe(), ::testing::HasSubstr("ExpectMessageFlow"));
}
...
TEST_F(M3MatcherCompositionTest, HappensBeforeWithM3Matchers) {
    ...
    EXPECT_THAT(matcher_with_events->describe(), ::testing::HasSubstr("HappensBefore"));
}
...
TEST_F(M3MatcherCompositionTest, MustNotSeeOnWithM3Matcher) {
    ...
    EXPECT_THAT(matcher_with_filter->describe(), ::testing::HasSubstr("MustNotSeeOn"));
}
...
TEST_F(M3MatcherCompositionTest, ComplexCompositionScenario) {
    ...
    auto latency_constraint =
        WithinLatency(ExpectMessageFlow("sender", "receiver"), std::chrono::milliseconds(100));
