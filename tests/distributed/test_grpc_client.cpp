...
TEST_F(GrpcClientTest, RegisterNodeRpc) {
    ...
    auto result = client_->register_node(
        request.node_id(),
        request.hostname(),
        request.grpc_port()
    );
...
TEST_F(GrpcClientTest, SendHeartbeatRpc) {
    ...
    auto hb_result = client_->send_heartbeat(
        "test-client-node-2",
        NodeState::RUNNING,
        clock_sync
    );
...
TEST_F(GrpcClientTest, SendHeartbeatTimeout) {
    ...
    if (timeout_client) {
        ...
        auto result = timeout_client->send_heartbeat(
            "test-node",
            NodeState::RUNNING,
            clock_sync
        );
...
TEST_F(GrpcClientTest, WaitBarrierTimeout) {
    ...
    auto result = client_->wait_barrier("barrier-timeout", "test-node", 100000);  // 100 microseconds
