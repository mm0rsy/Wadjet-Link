...
protected:
    ...
    void SetUp() override {
        ...
        try {
            ...
            channel_ = grpc::CreateChannel("127.0.0.1:15001", grpc::InsecureChannelCredentials());
...
TEST_F(GrpcServiceTest, WaitBarrierSynchronization) {
    ...
    // Register test nodes
    const std::vector<std::string> node_ids = {"barrier-node-1", "barrier-node-2",
                                               "barrier-node-3"};
    ...
    EXPECT_TRUE(barrier_status.ok() ||
                barrier_status.error_code() == grpc::StatusCode::DEADLINE_EXCEEDED);
}
