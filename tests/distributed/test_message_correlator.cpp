    ...
    void create_test_packets() {
        ...
        for (int i = 0; i < 5; ++i) {
            ...
            for (size_t j = 0; j < data.size(); ++j) {
                data[j] = static_cast<std::byte>(static_cast<unsigned int>(i * 16 + static_cast<int>(j)) % 256);
            }
...
TEST_F(MessageCorrelatorTest, AddPackets) {
    ...
    correlator.add_packets("node1", std::span<const Packet>(test_packets_.data(), test_packets_.size()));
...
TEST_F(MessageCorrelatorTest, MultiNodeCorrelation) {
    ...
    correlator.add_packets("node1", std::span<const Packet>(test_packets_.data(), test_packets_.size()));
    correlator.add_packets("node2", std::span<const Packet>(test_packets_.data(), test_packets_.size()));
...
TEST_F(MessageCorrelatorTest, ScalabilityManyNodes) {
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        correlator.add_packets(node_id, std::span<const Packet>(test_packets_.data(), test_packets_.size()));
    }
