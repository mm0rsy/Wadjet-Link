...
public:
    ...
    RtpsPacketBuilder& add_data_submessage(const EntityId& reader_id, const EntityId& writer_id,
    ...
    RtpsPacketBuilder& add_heartbeat_submessage(const EntityId& reader_id,
                                                const EntityId& writer_id, std::int64_t first_sn,
                                                std::int64_t last_sn, std::int32_t count) {
        Submessage submsg;
        submsg.writer_id = writer_id;
        submsg.reader_id = reader_id;
    ...
    RtpsPacketBuilder& add_acknack_submessage(const EntityId& reader_id, const EntityId& writer_id,
                                              std::int64_t base_sn, std::int32_t count) {
        Submessage submsg;
        submsg.reader_id = reader_id;
        submsg.writer_id = writer_id;
    ...
    RtpsPacketBuilder& add_gap_submessage(const EntityId& reader_id, const EntityId& writer_id,
                                          std::int64_t gap_start, std::int64_t gap_end) {
        Submessage submsg;
        submsg.reader_id = reader_id;
        submsg.writer_id = writer_id;
...
private:
    ...
    void add_data_submessage_bytes(std::vector<std::uint8_t>& packet, const Submessage& submsg) {
        ...
        // Reader entity ID
        for (auto b : submsg.reader_id.entity_key)
            packet.push_back(b);
        packet.push_back(static_cast<std::uint8_t>(submsg.reader_id.entity_kind));

        // Writer entity ID
        for (auto b : submsg.writer_id.entity_key)
            packet.push_back(b);
    ...
    void add_heartbeat_submessage_bytes(std::vector<std::uint8_t>& packet,
                                        const Submessage& submsg) {
    ...
    void add_info_dst_submessage_bytes(std::vector<std::uint8_t>& packet,
                                       const Submessage& submsg) {
    ...
    std::array<std::uint8_t, 12> guid_prefix_ = {0x01, 0x02, 0x03, 0x04, 0x05, 0x06,
...
TEST_F(DdsIntegrationTest, DecodeDataExchangePattern_ReliableWriter) {
    ...
    std::array<std::uint8_t, 12> writer_prefix = {0xAA, 0xBB, 0xCC, 0xDD, 0x11, 0x22,
    ...
    std::array<std::uint8_t, 12> reader_prefix = {0x11, 0x22, 0x33, 0x44, 0x55, 0x66,
...
TEST_F(DdsIntegrationTest, MultiVendor_FastDDS) {
    auto packet = RtpsPacketBuilder().set_version(2, 4).set_vendor(VendorId::FastDDS).build();
...
TEST_F(DdsIntegrationTest, MultiVendor_RTIConnext) {
    auto packet = RtpsPacketBuilder().set_version(2, 4).set_vendor(VendorId::RTI).build();
...
TEST_F(DdsIntegrationTest, MultiVendor_CycloneDDS) {
    auto packet = RtpsPacketBuilder().set_version(2, 4).set_vendor(VendorId::CycloneDDS).build();
...
TEST_F(DdsIntegrationTest, MultiVendor_OpenDDS) {
    auto packet = RtpsPacketBuilder().set_version(2, 4).set_vendor(VendorId::OpenDDS).build();
...
TEST_F(DdsIntegrationTest, MultiVendor_UnknownVendor) {
    ...
    auto packet =
        RtpsPacketBuilder().set_version(2, 4).set_vendor(static_cast<VendorId>(0x9999)).build();
...
TEST_F(DdsIntegrationTest, MultiVendor_CrossVendorCommunication) {
    ...
    std::array<std::uint8_t, 12> fast_dds_prefix = {0x01, 0x0F, 0xAA, 0xBB, 0xCC, 0xDD,
                                                    0x11, 0x22, 0x33, 0x44, 0x55, 0x66};

    std::array<std::uint8_t, 12> cyclone_prefix = {0x01, 0x05, 0x11, 0x22, 0x33, 0x44,
...
TEST_F(DdsIntegrationTest, Version_RTPS_2_2) {
    auto packet = RtpsPacketBuilder().set_version(2, 2).set_vendor(VendorId::RTI).build();
...
TEST_F(DdsIntegrationTest, Version_RTPS_2_3) {
    auto packet = RtpsPacketBuilder().set_version(2, 3).set_vendor(VendorId::OpenDDS).build();
...
TEST_F(DdsIntegrationTest, ErrorHandling_TruncatedSubmessage) {
    auto packet = RtpsPacketBuilder()
                      .add_data_submessage(make_entity_id(0, 0, 1, EntityKind::UserReader),
                                           make_entity_id(0, 0, 2, EntityKind::UserWriter), 1,
                                           {0x01, 0x02, 0x03, 0x04})
                      .build();
