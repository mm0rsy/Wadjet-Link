...
protected:
     ...
     */
    DistributedCaptureContext CreateMockCapture(const std::string& node_id,
...
TEST_F(DistributedAssertionsIntegrationTest, AssertionFailureReporting) {
    ...
    if (!result.matched) {
        EXPECT_FALSE(result.error_message.empty()) << "Failed assertion should have error message";
    }
}
...
TEST_F(DistributedAssertionsIntegrationTest, SequentialAssertions) {
    ...
    assertions.push_back({"A->B message flow", ExpectMessageFlow("node-a", "node-b")});
    assertions.push_back({"B->C message flow", ExpectMessageFlow("node-b", "node-c")});
    assertions.push_back({"A before B", HappensBefore("node-a", "node-b")});
    assertions.push_back({"B before C", HappensBefore("node-b", "node-c")});
