...
public:
    ...
    static std::filesystem::path create_temp_pcap(
        const std::string& name, const std::vector<std::vector<std::uint8_t>>& packets) {
        ...
        for (const auto& pkt : packets) {
            PacketView pkt_view(std::span<const std::byte>(
...
std::vector<std::uint8_t> build_someip_packet(std::uint16_t service_id, std::uint16_t method_id,
    ...
    pkt.insert(pkt.end(), {
                              ...
                              0x00, 0x01, 0x40, 0x00, 0x40, 0x11, 0x00, 0x00,  // TTL, UDP, Checksum
...
std::vector<std::uint8_t> build_doip_packet(doip::PayloadType payload_type,
                                            const std::vector<std::uint8_t>& payload) {
    std::vector<std::uint8_t> pkt;

    // Ethernet header
    pkt.insert(pkt.end(), {0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xAA, 0xBB,
                           0x08, 0x00});
...
protected:
    ...
    void TearDown() override { PcapTestFixture::cleanup(); }
...
TEST_F(PcapDecodeTest, DecodeDoIPPacket) {
    ...
    auto pkt = build_doip_packet(doip::PayloadType::DiagnosticMessage,
...
TEST_F(PcapRoundTripTest, WriteAndReadBack) {
    ...
    std::vector<std::uint8_t> pkt1 = {
        0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xAA, 0xBB, 0x08, 0x00,  // IPv4
        0x45, 0x00, 0x00, 0x1C, 0x00, 0x01, 0x40, 0x00, 0x40, 0x11, 0x00, 0x00, 0xC0, 0xA8,
        0x01, 0x01, 0xC0, 0xA8, 0x01, 0x02, 0x04, 0xD2, 0x04, 0xD2, 0x00, 0x08, 0x00, 0x00};
    ...
    // Write
    {
        ...
        PacketView pkt_view(std::span<const std::byte>(
                                reinterpret_cast<const std::byte*>(pkt1.data()), pkt1.size()),
...
TEST_F(PcapRoundTripTest, PreservesTimestamps) {
    ...
    std::vector<std::uint8_t> pkt = {0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66,
                                     0x77, 0x88, 0x99, 0xAA, 0xBB, 0x08, 0x00};
