...
protected:
    std::span<const std::byte> as_span(const std::vector<std::uint8_t>& data) {
        return std::span<const std::byte>(reinterpret_cast<const std::byte*>(data.data()),
                                          data.size());
    }
    ...
    ProtocolLayer make_layer(const std::string& name, std::size_t offset, std::size_t header_len,
        ...
        return ProtocolLayer{.name = name,
                             .offset = offset,
...
TEST_F(ProtocolCompletenessIntegrationTest, SimpleEthernetIPv4TCPStack) {
    ...
    std::vector<ProtocolLayer> layers = {make_layer("Ethernet", 0, 14, 242),
...
TEST_F(ProtocolCompletenessIntegrationTest, IPv4UDPSomeIP) {
    ...
    std::vector<ProtocolLayer> layers = {
        make_layer("Ethernet", 0, 14, 500), make_layer("IPv4", 14, 20, 486, 0x0800),
        make_layer("UDP", 34, 8, 478, 17), make_layer("SOME/IP", 42, 16, 462)};
...
TEST_F(ProtocolCompletenessIntegrationTest, TCPWithDoIP) {
    ...
    std::vector<ProtocolLayer> layers = {
        make_layer("Ethernet", 0, 14, 100), make_layer("IPv4", 14, 20, 86, 0x0800),
        make_layer("TCP", 34, 20, 66, 6), make_layer("DoIP", 54, 8, 58)};
...
TEST_F(ProtocolCompletenessIntegrationTest, SomeIPSDOverUDP) {
    ...
    std::vector<ProtocolLayer> layers = {
        make_layer("Ethernet", 0, 14, 300), make_layer("IPv4", 14, 20, 286, 0x0800),
        make_layer("UDP", 34, 8, 278, 17), make_layer("SOME/IP-SD", 42, 24, 254)};
...
TEST_F(ProtocolCompletenessIntegrationTest, UDSOverDoIPOverTCP) {
    ...
    std::vector<ProtocolLayer> layers = {
        make_layer("Ethernet", 0, 14, 200), make_layer("IPv4", 14, 20, 186, 0x0800),
        make_layer("TCP", 34, 20, 166, 6), make_layer("DoIP", 54, 8, 158),
...
TEST_F(ProtocolCompletenessIntegrationTest, VLANTaggedStack) {
    ...
    std::vector<ProtocolLayer> layers = {make_layer("Ethernet", 0, 18, 238),  // +4 for VLAN tag
...
TEST_F(ProtocolCompletenessIntegrationTest, VLANWithDiagnosticStack) {
    ...
    std::vector<ProtocolLayer> layers = {
        make_layer("Ethernet", 0, 18, 250), make_layer("IPv4", 18, 20, 232, 0x0800),
        make_layer("TCP", 38, 20, 212, 6), make_layer("DoIP", 58, 8, 204),
...
TEST_F(ProtocolCompletenessIntegrationTest, LengthConsistency) {
    ...
    std::vector<ProtocolLayer> layers = {
        make_layer("Ethernet", 0, 14, 242), make_layer("IPv4", 14, 20, 222, 0x0800),
        make_layer("TCP", 34, 20, 202, 6), make_layer("DoIP", 54, 8, 194),
...
TEST_F(ProtocolCompletenessIntegrationTest, LengthValidationWithPadding) {
    ...
    std::vector<ProtocolLayer> layers = {
        make_layer("Ethernet", 0, 14, 100), make_layer("IPv4", 14, 20, 86, 0x0800),
        make_layer("UDP", 34, 8, 78, 17), make_layer("SOME/IP", 42, 16, 62)};
...
TEST_F(ProtocolCompletenessIntegrationTest, IPv4ChecksumValidation) {
    ...
    std::vector<ProtocolLayer> layers = {make_layer("Ethernet", 0, 14, 86),
...
TEST_F(ProtocolCompletenessIntegrationTest, UDPChecksumWithPseudoHeader) {
    ...
    std::vector<ProtocolLayer> layers = {
        make_layer("Ethernet", 0, 14, 136), make_layer("IPv4", 14, 20, 116, 0x0800, 0x0000, true),
        make_layer("UDP", 34, 8, 108, 17, 0x0000, true), make_layer("SOME/IP", 42, 16, 92)};
...
TEST_F(ProtocolCompletenessIntegrationTest, TCPChecksumWithPseudoHeader) {
    ...
    std::vector<ProtocolLayer> layers = {
        make_layer("Ethernet", 0, 14, 106), make_layer("IPv4", 14, 20, 86, 0x0800, 0x0000, true),
        make_layer("TCP", 34, 20, 66, 6, 0x0000, true), make_layer("DoIP", 54, 8, 58)};
...
TEST_F(ProtocolCompletenessIntegrationTest, IPv4FragmentationWithPayload) {
    ...
    // First fragment
    std::vector<ProtocolLayer> frag1 = {make_layer("Ethernet", 0, 14, 1500),
    ...
    // Second fragment
    std::vector<ProtocolLayer> frag2 = {make_layer("Ethernet", 0, 14, 1500),
                                        make_layer("IPv4", 14, 20, 1486, 0x0800)};
...
TEST_F(ProtocolCompletenessIntegrationTest, StrictVsLenientModes) {
    // Strict mode - fails on invalid sequence
    {
        ...
        std::vector<ProtocolLayer> invalid = {make_layer("Ethernet", 0, 14, 100),
    ...
    // Lenient mode - continues despite errors
    {
        ...
        std::vector<ProtocolLayer> layers = {make_layer("Ethernet", 0, 14, 100),
...
TEST_F(ProtocolCompletenessIntegrationTest, SingleLayer) {
    ...
    std::vector<ProtocolLayer> single = {make_layer("Ethernet", 0, 14, 0)};
...
TEST_F(ProtocolCompletenessIntegrationTest, VeryLargePacket) {
    ...
    std::vector<ProtocolLayer> layers = {
        make_layer("Ethernet", 0, 14, 8986), make_layer("IPv4", 14, 20, 8966, 0x0800),
        make_layer("UDP", 34, 8, 8958, 17), make_layer("SOME/IP", 42, 16, 8942)};
...
TEST_F(ProtocolCompletenessIntegrationTest, VehicleSomeIPSDMulticast) {
    ...
    std::vector<ProtocolLayer> sd_discovery = {
        make_layer("Ethernet", 0, 14, 500), make_layer("IPv4", 14, 20, 480, 0x0800),
        make_layer("UDP", 34, 8, 472, 17), make_layer("SOME/IP-SD", 42, 24, 448)};
...
TEST_F(ProtocolCompletenessIntegrationTest, DiagnosticRequestResponse) {
    ...
    std::vector<ProtocolLayer> request = {
        make_layer("Ethernet", 0, 14, 86), make_layer("IPv4", 14, 20, 72, 0x0800),
        make_layer("TCP", 34, 20, 52, 6), make_layer("DoIP", 54, 8, 44),
    ...
    std::vector<ProtocolLayer> response = {
        make_layer("Ethernet", 0, 14, 120), make_layer("IPv4", 14, 20, 106, 0x0800),
        make_layer("TCP", 34, 20, 86, 6), make_layer("DoIP", 54, 8, 78),
...
TEST_F(ProtocolCompletenessIntegrationTest, TesterPresentKeepalive) {
    ...
    std::vector<ProtocolLayer> keepalive = {
        make_layer("Ethernet", 0, 14, 54), make_layer("IPv4", 14, 20, 40, 0x0800),
        make_layer("TCP", 34, 20, 20, 6), make_layer("DoIP", 54, 8, 12),
