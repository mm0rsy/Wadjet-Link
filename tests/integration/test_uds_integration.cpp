...
public:
    ...
    static std::vector<std::uint8_t> build_diagnostic_message(
        std::uint16_t source_addr, std::uint16_t target_addr,
    ...
    static std::vector<std::uint8_t> uds_diagnostic_session_control(SessionType session_type,
                                                                    bool spr = false) {
        std::uint8_t sub_function = static_cast<std::uint8_t>(session_type);
        if (spr)
            sub_function |= 0x80;  // Suppress positive response
    ...
    static std::vector<std::uint8_t> uds_diagnostic_session_control_response(
        SessionType session_type, std::uint16_t p2_time = 50, std::uint16_t p2_star_time = 5000) {
        return {0x50,  // Positive response (0x10 + 0x40)
    ...
    static std::vector<std::uint8_t> uds_security_access_seed_response(
        std::uint8_t level, const std::vector<std::uint8_t>& seed) {
    ...
    static std::vector<std::uint8_t> uds_security_access_send_key(
        std::uint8_t level, const std::vector<std::uint8_t>& key) {
        std::vector<std::uint8_t> request = {
            0x27, static_cast<std::uint8_t>(level + 1)};  // Even level = send key
    ...
    static std::vector<std::uint8_t> uds_tester_present_response() { return {0x7E, 0x00}; }
    ...
    static std::vector<std::uint8_t> uds_read_data_by_identifier_response(
        std::uint16_t did, const std::vector<std::uint8_t>& data) {
        std::vector<std::uint8_t> response = {0x62,  // 0x22 + 0x40
    ...
    static std::vector<std::uint8_t> uds_negative_response(ServiceID rejected_sid, NRC nrc) {
        return {0x7F, static_cast<std::uint8_t>(rejected_sid), static_cast<std::uint8_t>(nrc)};
    }
...
TEST_F(UdsDecoderIntegrationTest, DecodeNegativeResponse) {
    auto uds_data = DoIpUdsPacketBuilder::uds_negative_response(ServiceID::SecurityAccess,
...
TEST_F(UdsDecoderIntegrationTest, DecodeResponsePending) {
    auto uds_data = DoIpUdsPacketBuilder::uds_response_pending(ServiceID::RoutineControl);
...
TEST_F(UdsSessionIntegrationTest, SessionTransition_ExtendedToProgramming) {
    ...
    auto prog_req =
        DoIpUdsPacketBuilder::uds_diagnostic_session_control(SessionType::ProgrammingSession);
...
TEST_F(UdsSessionIntegrationTest, SecurityUnlock_Level1) {
    ...
    auto seed_resp =
        DoIpUdsPacketBuilder::uds_security_access_seed_response(0x01, {0x12, 0x34, 0x56, 0x78});
    ...
    // Send key
    auto key_req =
        DoIpUdsPacketBuilder::uds_security_access_send_key(0x01, {0xAB, 0xCD, 0xEF, 0x01});
...
TEST_F(UdsSessionIntegrationTest, SecurityUnlock_InvalidKey) {
    ...
    auto key_req =
        DoIpUdsPacketBuilder::uds_security_access_send_key(0x01, {0x00, 0x00, 0x00, 0x00});
    std::span<const std::uint8_t> key_req_span(key_req.data(), key_req.size());
    session_.process_message(key_req_span, true);

    auto nrc =
        DoIpUdsPacketBuilder::uds_negative_response(ServiceID::SecurityAccess, NRC::InvalidKey);
...
TEST_F(DoIpUdsIntegrationTest, DecodeDoIpDiagnosticMessage_WithUds) {
    ...
    auto doip_packet = DoIpUdsPacketBuilder::build_diagnostic_message(0x0E00,  // Tester address
                                                                      0x0001,  // ECU address
                                                                      uds_payload);
...
TEST_F(DoIpUdsIntegrationTest, DecodeDoIpDiagnosticResponse_WithUds) {
    ...
    auto doip_packet =
        DoIpUdsPacketBuilder::build_diagnostic_message(0x0001,  // ECU address (responding)
...
TEST_F(DoIpUdsIntegrationTest, DecodeDoIpDiagnosticNRC_WithUds) {
    auto uds_payload = DoIpUdsPacketBuilder::uds_negative_response(ServiceID::WriteDataByIdentifier,
                                                                   NRC::SecurityAccessDenied);
    auto doip_packet = DoIpUdsPacketBuilder::build_diagnostic_message(0x0001, 0x0E00, uds_payload);
