...
static Packet create_ethernet_packet(Timestamp ts, std::size_t size = 64) {
    ...
    for (int i = 0; i < 6; ++i) data.push_back(std::byte{0xff});
...
static bool create_test_pcap(const fs::path& path, std::size_t num_packets,
    ...
    for (std::size_t i = 0; i < num_packets; ++i) {
        ...
        if (!writer.write_packet(pkt)) return false;
...
protected:
    ...
    fs::path test_file(const std::string& name) const {
...
TEST_F(ReplaySessionTest, RunWithModifier) {
    ...
    int packet_index = 0;
    session.set_modifier([&packet_index]([[maybe_unused]] Packet& pkt) {
        return (packet_index++ % 2) == 0;
    });
...
TEST_F(ReplaySessionTest, RunWithProgressCallback) {
    ...
    session.set_progress_callback([&callback_count](const io::ReplayStats& stats) {
        callback_count++;
...
TEST_F(ReplaySessionTest, Stop) {
    ...
    std::thread replay_thread([&session]() {
        session.run();
    });
