...
std::vector<std::byte> make_data_submessage(std::uint16_t extra_flags = 0,
    ...
    std::vector<std::byte> submsg = make_bytes(0x15,       // DATA submessage kind
    ...
    auto body = make_bytes(
        // Extra flags (2 bytes)
        static_cast<uint8_t>(extra_flags & 0xFF), static_cast<uint8_t>((extra_flags >> 8) & 0xFF),
...
std::vector<std::byte> make_heartbeat_submessage(std::uint32_t first_sn = 1,
                                                 std::uint32_t last_sn = 10,
                                                 std::int32_t count = 5) {
    // Submessage header
    std::vector<std::byte> submsg = make_bytes(0x07,       // HEARTBEAT submessage kind
    ...
    auto body = make_bytes(
        ...
        0x00, 0x00, 0x00, 0x00, static_cast<uint8_t>(first_sn), 0x00, 0x00, 0x00,
        // Last Sequence Number (8 bytes)
        0x00, 0x00, 0x00, 0x00, static_cast<uint8_t>(last_sn), 0x00, 0x00, 0x00,
        // Count (4 bytes)
...
std::vector<std::byte> make_info_ts_submessage(std::int32_t seconds = 1000,
                                               std::uint32_t fraction = 500000) {
    // Submessage header
    std::vector<std::byte> submsg = make_bytes(0x09,  // INFO_TS submessage kind
    ...
    // Timestamp (8 bytes)
    auto body = make_bytes(
        static_cast<uint8_t>(seconds & 0xFF), static_cast<uint8_t>((seconds >> 8) & 0xFF),
        static_cast<uint8_t>((seconds >> 16) & 0xFF), static_cast<uint8_t>((seconds >> 24) & 0xFF),
        static_cast<uint8_t>(fraction & 0xFF), static_cast<uint8_t>((fraction >> 8) & 0xFF),
...
std::vector<std::byte> make_info_dst_submessage() {
    // Submessage header
    std::vector<std::byte> submsg = make_bytes(0x0E,       // INFO_DST submessage kind
                                               0x01,       // Flags: endian=little
                                               0x0C, 0x00  // Length = 12
    );

    // GUID Prefix (12 bytes)
    auto body = make_bytes(0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xAA, 0xBB, 0xCC);
...
TEST_F(RtpsDecoderTest, RejectInvalidMagic) {
    auto msg = make_bytes('R', 'T', 'P', 'X',  // Invalid magic
                          0x02, 0x04, 0x01, 0x01, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08,
                          0x09, 0x0A, 0x0B, 0x0C);
    set_data(msg);
...
protected:
    ...
    std::vector<std::byte> make_rtps_packet() {
        ...
        auto ip_header = make_bytes(0x45,        // Version=4, IHL=5
                                    0x00,        // DSCP/ECN
        ...
        // UDP header (8 bytes)
        auto udp = make_bytes(0x1C, 0xF8,  // Source port: 7416
