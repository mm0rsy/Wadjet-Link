...
DecodeContext make_context(const Container& data) {
    DecodeContext ctx;
    ctx.data = std::span<const std::byte>(
...
TEST_F(UDPDecoderTest, RejectInvalidLength) {
    std::array<std::uint8_t, 8> bad_length = {
        0x30, 0x39, 0x00, 0x50,
        0x00, 0x05,  // Length: 5 (invalid, min is 8)
        0x00, 0x00
    };
...
TEST_F(SomeIpDecoderTest, RejectInvalidLength) {
    std::array<std::uint8_t, 16> bad_length = {
        0x12, 0x34, 0x80, 0x01,
        0x00, 0x00, 0x00, 0x04, // Length: 4 (invalid, min is 8)
        0x00, 0x01, 0x00, 0x0A,
        0x01, 0x01, 0x02, 0x00
    };
    auto ctx = make_context(bad_length);
...
TEST_F(DoIPDecoderTest, ParseRoutingActivationRequest) {
    ...
    std::span<const std::byte> payload{
        reinterpret_cast<const std::byte*>(doip_routing_activation.data() + 8),
        7
    };
...
protected:
    std::array<std::uint8_t, 16> test_data = {
        0x01, 0x02, 0x03, 0x04,
        0x05, 0x06, 0x07, 0x08,
        0x09, 0x0A, 0x0B, 0x0C,
        0x0D, 0x0E, 0x0F, 0x10
