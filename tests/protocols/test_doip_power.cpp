...
TEST_F(DoIPPowerModeTest, PowerModeEnumConsistency) {
    ...
    EXPECT_NE(PayloadType::DiagnosticPowerModeRequest, PayloadType::DiagnosticPowerModeResponse);
}
...
TEST_F(DoIPEntityStatusTest, EntityStatusEnumConsistency) {
    EXPECT_NE(PayloadType::DoIPEntityStatusRequest, PayloadType::DoIPEntityStatusResponse);
}
...
TEST_F(DoIPParseEntityStatusTest, ParseEntityStatusTooShort) {
    std::array<std::byte, 4> payload{std::byte(0x00), std::byte(0x0A), std::byte(0x05),
                                     std::byte(0x00)};
    auto result = DoIPDecoder::parse_entity_status(payload);
