...
TEST_F(SomeIpSdOptionLinkingTest, OptionIndexValidation) {
    ...
    EXPECT_LT(entry.index1_first_option + entry.num_options_1,
              static_cast<int>(options.size()) + 1);
    EXPECT_LT(entry.index2_first_option + entry.num_options_2,
              static_cast<int>(options.size()) + 1);
}
...
TEST_F(SomeIpSdComplexLinkingTest, EventgroupWithTwoOptionSets) {
    ...
    auto opts = options.get_options_for_entry(entry.index1_first_option, entry.num_options_1,
