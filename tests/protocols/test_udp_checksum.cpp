...
protected:
    ...
    static std::vector<std::byte> create_udp_packet(const std::string& /*src_ip*/,
        ...
        // Source IP
        packet.insert(packet.end(), {std::byte{0xC0}, std::byte{0xA8}, std::byte{0x01},
                                     std::byte{0x01}});  // 192.168.1.1
        // Destination IP
        packet.insert(packet.end(), {std::byte{0xC0}, std::byte{0xA8}, std::byte{0x01},
                                     std::byte{0x02}});  // 192.168.1.2
...
TEST_F(UdpChecksumTest, InvalidChecksum) {
    auto packet =
        create_udp_packet("192.168.1.1", "192.168.1.2", 1234, 5678, {0x48, 0x65, 0x6C, 0x6C, 0x6F},
...
TEST_F(UdpChecksumTest, ZeroChecksumIPv4) {
    auto packet = create_udp_packet("192.168.1.1", "192.168.1.2", 1234, 5678, {0x00, 0x00, 0x00},
                                    0x0000, true);
...
TEST_F(UdpChecksumTest, EmptyPayload) {
    auto packet = create_udp_packet("192.168.1.1", "192.168.1.2", 53, 53, {});  // Empty payload
...
TEST_F(UdpChecksumTest, LargePayload) {
    ...
    auto packet = create_udp_packet("192.168.1.1", "192.168.1.2", 1234, 5678, large_payload);
