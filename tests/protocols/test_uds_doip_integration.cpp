...
std::vector<std::byte> create_doip_diagnostic_message(LogicalAddress source, LogicalAddress target,
...
std::vector<std::byte> create_dsc_request(uds::SessionType session) {
    return {std::byte{0x10},  // SID: DiagnosticSessionControl
            static_cast<std::byte>(session)};
}

/// @brief Create UDS DiagnosticSessionControl positive response
std::vector<std::byte> create_dsc_response(uds::SessionType session, std::uint16_t p2_ms = 50,
                                           std::uint16_t p2_star_10ms = 500) {
    return {std::byte{0x50},  // SID + 0x40
            static_cast<std::byte>(session),
...
std::vector<std::byte> create_security_access_seed_response(std::uint8_t level,
...
std::vector<std::byte> create_security_access_key_request(std::uint8_t level,
...
std::vector<std::byte> create_security_access_key_response(std::uint8_t level) {
    return {std::byte{0x67},  // SID + 0x40
            static_cast<std::byte>(level + 1)};
}

/// @brief Create UDS negative response
std::vector<std::byte> create_negative_response(uds::ServiceID rejected_sid, uds::NRC nrc) {
    return {std::byte{0x7F}, static_cast<std::byte>(rejected_sid), static_cast<std::byte>(nrc)};
}

/// @brief Create UDS TesterPresent request
std::vector<std::byte> create_tester_present_request(bool suppress = false) {
    return {std::byte{0x3E},  // SID: TesterPresent
...
TEST_F(RequestCorrelatorTest, RecordAndMatchSimpleRequest) {
    ...
    correlator_.record_request(
        req_header, uds::DiagnosticSessionControlRequest{uds::SessionType::DefaultSession},
        req_transport);
    ...
    auto pair = correlator_.process_response(
        resp_header,
        uds::DiagnosticSessionControlResponse{uds::SessionType::DefaultSession, 50, 500},
        resp_transport);
...
TEST_F(RequestCorrelatorTest, MatchNegativeResponse) {
    ...
    auto pair =
        correlator_.process_response(resp_header,
                                     uds::NegativeResponseMessage{uds::ServiceID::SecurityAccess,
...
TEST_F(RequestCorrelatorTest, HandleResponsePending) {
    ...
    pending_header.negative_response_code =
    ...
    auto pending_pair = correlator_.process_response(
        pending_header,
        uds::NegativeResponseMessage{uds::ServiceID::RoutineControl,
    ...
    auto pair =
        correlator_.process_response(final_header, uds::RoutineControlResponse{}, final_transport);
...
protected:
    ...
    void SetUp() override {
        manager_.on_event(
            [this](DiagnosticEvent event, [[maybe_unused]] const auto& state,
                   [[maybe_unused]] auto* pair) { received_events_.push_back(event); });
    }
};
...
TEST_F(DiagnosticSessionManagerTest, TrackSecurityAccessSequence) {
    ...
    manager_.process_doip_raw(create_doip_diagnostic_message(TESTER, ECU, dsc_req),
                              std::chrono::steady_clock::now());
    manager_.process_doip_raw(create_doip_diagnostic_message(ECU, TESTER, dsc_resp),
                              std::chrono::steady_clock::now());

    // Security Access - Request Seed
    auto seed_req = create_security_access_seed_request(0x01);
    manager_.process_doip_raw(create_doip_diagnostic_message(TESTER, ECU, seed_req),
                              std::chrono::steady_clock::now());

    // Security Access - Seed Response
    std::vector<std::byte> seed = {std::byte{0x12}, std::byte{0x34}, std::byte{0x56},
                                   std::byte{0x78}};
    auto seed_resp = create_security_access_seed_response(0x01, seed);
    manager_.process_doip_raw(create_doip_diagnostic_message(ECU, TESTER, seed_resp),
                              std::chrono::steady_clock::now());

    // Security Access - Send Key
    std::vector<std::byte> key = {std::byte{0xAB}, std::byte{0xCD}, std::byte{0xEF},
                                  std::byte{0x01}};
    auto key_req = create_security_access_key_request(0x01, key);
    manager_.process_doip_raw(create_doip_diagnostic_message(TESTER, ECU, key_req),
                              std::chrono::steady_clock::now());

    // Security Access - Key Accepted
    auto key_resp = create_security_access_key_response(0x01);
    manager_.process_doip_raw(create_doip_diagnostic_message(ECU, TESTER, key_resp),
                              std::chrono::steady_clock::now());
...
TEST_F(DiagnosticSessionManagerTest, HandleNegativeResponse) {
    ...
    manager_.process_doip_raw(create_doip_diagnostic_message(TESTER, ECU, req_uds),
                              std::chrono::steady_clock::now());

    // Send negative response (conditions not correct)
    auto neg_resp = create_negative_response(uds::ServiceID::DiagnosticSessionControl,
                                             uds::NRC::ConditionsNotCorrect);
    manager_.process_doip_raw(create_doip_diagnostic_message(ECU, TESTER, neg_resp),
                              std::chrono::steady_clock::now());
...
TEST_F(DiagnosticSessionManagerTest, TesterPresentKeepsSessionAlive) {
    ...
    manager_.process_doip_raw(create_doip_diagnostic_message(TESTER, ECU, tp_req),
                              std::chrono::steady_clock::now());

    auto tp_resp = create_tester_present_response();
    manager_.process_doip_raw(create_doip_diagnostic_message(ECU, TESTER, tp_resp),
                              std::chrono::steady_clock::now());
...
TEST_F(DiagnosticSessionManagerTest, Statistics) {
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        manager_.process_doip_raw(create_doip_diagnostic_message(TESTER, ECU, req),
                                  std::chrono::steady_clock::now());
        manager_.process_doip_raw(create_doip_diagnostic_message(ECU, TESTER, resp),
                                  std::chrono::steady_clock::now());
    }
...
TEST_F(UdsOverDoipDecoderTest, DecodeNegativeResponse) {
    auto uds_data = create_negative_response(uds::ServiceID::SecurityAccess, uds::NRC::InvalidKey);
    auto packet = create_doip_diagnostic_message(ECU, TESTER, uds_data);
...
TEST_F(UdsOverDoipDecoderTest, LooksLikeDiagnosticMessage) {
    ...
    std::vector<std::byte> other = {
        std::byte{0x02}, std::byte{0xFD}, std::byte{0x00}, std::byte{0x01},  // Not diagnostic
