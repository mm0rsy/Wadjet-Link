...
namespace rdbi_vin {
    ...
    // Response with VIN
    const std::vector<std::uint8_t> response = {
        0x62, 0xF1, 0x90,     // RDBI response + DID
...
TEST_F(UdsRegressionTest, DSC_ConditionsNotCorrect) {
    ...
    EXPECT_EQ(result->header.negative_response_code, static_cast<std::uint8_t>(NRC::ConditionsNotCorrect));
...
TEST_F(UdsRegressionTest, SA_ExceededAttempts) {
    ...
    ASSERT_TRUE(result.is_ok());
    EXPECT_TRUE(result->header.is_negative_response());
    EXPECT_EQ(result->header.negative_response_code, static_cast<std::uint8_t>(NRC::ExceededNumberOfAttempts));
}
...
TEST_F(UdsRegressionTest, SA_TimeDelayNotExpired) {
    ...
    ASSERT_TRUE(result.is_ok());
    EXPECT_TRUE(result->header.is_negative_response());
    EXPECT_EQ(result->header.negative_response_code, static_cast<std::uint8_t>(NRC::RequiredTimeDelayNotExpired));
}
...
TEST_F(UdsRegressionTest, WDBI_SecurityDenied) {
    ...
    ASSERT_TRUE(result.is_ok());
    EXPECT_TRUE(result->header.is_negative_response());
    EXPECT_EQ(result->header.negative_response_code, static_cast<std::uint8_t>(NRC::SecurityAccessDenied));
}
...
TEST_F(UdsRegressionTest, TransferData_WrongSequence) {
    ...
    ASSERT_TRUE(result.is_ok());
    EXPECT_TRUE(result->header.is_negative_response());
    EXPECT_EQ(result->header.negative_response_code, static_cast<std::uint8_t>(NRC::WrongBlockSequenceCounter));
}
...
TEST_F(UdsRegressionTest, ResponsePending_ForRoutine) {
    ...
    ASSERT_TRUE(result.is_ok());
    EXPECT_TRUE(result->header.is_negative_response());
    EXPECT_EQ(result->header.negative_response_code, static_cast<std::uint8_t>(NRC::RequestCorrectlyReceivedResponsePending));
...
TEST_F(UdsRegressionTest, AllStandardNRCs) {
    ...
    for (const auto& tc : test_cases) {
        ...
        EXPECT_EQ(result->header.negative_response_code, std::optional<std::uint8_t>(static_cast<std::uint8_t>(tc.expected)))
