...
TEST_F(PerformanceRegressionTest, PerformanceOverhead) {
    ...
    std::vector<protocols::ProtocolLayer> layers;
    for (int i = 0; i < 10; i++) {
        layers.push_back(protocols::ProtocolLayer{
            .name = "Layer",
    ...
    double ops_per_ms = 1000.0 / static_cast<double>(duration_ms.count() > 0 ? duration_ms.count() : 1);
    ...
    // Test with malformed inputs
    {
        ...
        auto result = strict_validator.validateChecksums(
            std::span<const std::byte>(
                reinterpret_cast<const std::byte*>(packet_data.data()),
                packet_data.size()
            ),
