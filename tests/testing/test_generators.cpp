...
TEST_F(EthernetBuilderTest, BuildPacketCreatesPacketObject) {
    auto packet = EthernetBuilder(rng_)
        .with_ethertype(0x0800)
        .build_packet();
...
TEST_F(SOMEIPBuilderTest, BuildsValidSOMEIPHeader) {
    auto message = SOMEIPBuilder(rng_)
        .with_service_id(0x1234)
        .with_method_id(0x5678)
        .build();
...
TEST_F(SOMEIPBuilderTest, SetsMessageType) {
    auto message = SOMEIPBuilder(rng_)
        .with_message_type(protocols::someip::MessageType::Response)
        .build();
...
TEST_F(SOMEIPBuilderTest, RandomMessageTypeIsValid) {
    ...
    for (int i = 0; i < 100; ++i) {
        auto message = SOMEIPBuilder(rng)
...
TEST_F(PacketGeneratorTest, GeneratesSOMEIPServicePacket) {
    auto packet = gen_.someip_service_packet(0xABCD, 
