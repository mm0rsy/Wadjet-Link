...
TEST_F(RecordedStreamTest, FilterWithPredicate) {
    ...
    auto udp_only = stream.filter([](const Packet& pkt) { return ::testing::Value(pkt, IsUDP()); });
...
TEST_F(RecordedStreamTest, NoneMatch) {
    ...
    EXPECT_TRUE(
        udp_stream.none_match([](const Packet& pkt) { return ::testing::Value(pkt, IsTCP()); }));
}
...
TEST_F(RecordedStreamTest, FindIndex) {
    ...
    auto tcp_idx =
        stream.find_index([](const Packet& pkt) { return ::testing::Value(pkt, IsTCP()); });
...
TEST_F(RecordedStreamTest, TakeWhile) {
    ...
    auto first_udps =
        stream.take_while([](const Packet& pkt) { return ::testing::Value(pkt, IsUDP()); });
...
TEST_F(RecordedStreamTest, SkipWhile) {
    ...
    auto after_first_udps =
        stream.skip_while([](const Packet& pkt) { return ::testing::Value(pkt, IsUDP()); });
...
protected:
    ...
    void send_test_packets(int count) {
        for (int i = 0; i < count; ++i) {
            std::vector<std::uint8_t> payload = {static_cast<std::uint8_t>(i), 0xAA, 0xBB};
...
TEST_F(RecordSessionTest, RecordUntilPredicate) {
    ...
    int packet_count = 0;
    bool found =
        recorder.record_until([&packet_count](const Packet&) { return ++packet_count >= 3; },
...
TEST_F(RecordSessionTest, RecordNMatching) {
    ...
    bool found = recorder.record_n_matching(
        5, [](const Packet& pkt) { return ::testing::Value(pkt, IsUDP()); },
        std::chrono::milliseconds(500));
